...
app.all("/query", function (req, res, next) {
  ...
  for (
    var queryId = 0;
    queryId < req.body.targets.length && !error;
    queryId++
  ) {
    ...
    if (queryArgs.err != null) {
      ...
      runAggregateQuery(requestId, queryId, body, queryArgs, res, next);
    }
  }
});
  ...
  if (openBracketIndex == -1) {
    ...
    if (closeBracketIndex == -1) {
      ...
      if (doc.operation == "aggregate") {
        ...
        queryErrors.push(
          "Unknown operation " + doc.operation + ", only aggregate supported"
        );
      }
    }
  }
...
function intervalCount(range, intervalString, intervalMs) {
  ...
  var output =
    intervalsInRange.toFixed(0) + " " + intervalString + " intervals";
  return output;
}
